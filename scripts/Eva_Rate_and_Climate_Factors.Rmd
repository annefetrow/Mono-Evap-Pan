...
eva_rate_cal <- function(h, date_time) {
  ...
  for (i in 1:(length(h) - 2)) {
    ...
    # Set rate to zero if the absolute value is greater than 10
    if (abs(rate) > 3) {
      rate <- 0
    }
...
plotByMonth <- function(T_water_all_date_time, T_water_all, T_air_all_date_time, T_air_all, RH_all_date_time, RH_all) {
  ...
  Variable = factor(
    ...
    # Define a new linetype mapping for each variable
linetype_mapping <- c("Water Temp" = "dashed", "Air Temp" = "dashed", "RH" = "solid")
    
    # Create the individual plot
p <- ggplot(plot_data, aes(x = Date_time, y = Value, color = Variable, linetype = Variable)) +
  geom_line() +
  scale_y_continuous(
    name = "T (C)",
  ...
  scale_x_datetime(
        ...
        date_breaks = "5 days",  # Consistent break interval
        date_labels = "%b %d"  # Date label format
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12),  # Center the title and set font size
    legend.position = "none",  # Hide the legend
    axis.text.x = element_text(angle = 0, hjust = 1, size = 8),  # Set font size for x-axis text
...
plotMonthlyEvapWaterAir <- function(eva_rate, water_temp_data, air_temp_data) {
  ...
  # Calculate overall min and max for temperature and evaporation for uniform y-axis across plots
  all_temps <- c(trimmed_temp, trimmed_T_air)
  min_temp <- floor(min(all_temps, na.rm = TRUE))
  max_temp <- ceiling(max(all_temps, na.rm = TRUE))
  
  min_evap <- floor(min(trimmed_eva_rate, na.rm = TRUE))
  max_evap <- ceiling(max(trimmed_eva_rate, na.rm = TRUE))
  ...
  for (i in 1:num_months) { 
    ...
    temp_data$temp_value_scaled <- rescale(temp_data$temp_value, to = c(min_evap, max_evap)) # Scale for plotting
    ...
    air_data$air_value_scaled <- rescale(air_data$air_value, to = c(min_evap, max_evap)) # Scale for plotting
    ...
    # Define x-axis date range for uniformity across monthly plots
    month_start <- as.POSIXct(paste(currentYear, currentMonth, "01", sep = "-"))
    month_end <- as.POSIXct(paste(currentYear, currentMonth, days_in_month(month_start), sep = "-")) + 
        hours(23) + minutes(59) + seconds(59)
    ...
    p <- ggplot() +
      ...
      geom_line(data = temp_data, aes(x = temp_time, y = temp_value_scaled, color = "Water Temp"), linetype = "dashed") +
      # Line plot for Air Temperature (dashed)
      geom_line(data = air_data, aes(x = air_time, y = air_value_scaled, color = "Air Temp"), linetype = "dashed") +
      # Independent y-axis scales for evaporation and temperatures
      scale_y_continuous(
        name = "E (mm/hr)",
        limits = c(min_evap, max_evap),
        sec.axis = sec_axis(
          trans = ~ rescale(., from = c(min_evap, max_evap), to = c(min_temp, max_temp)),
          name = "T (C)"
        )
      ) +
