...
water_level_data_import  <- function(filename) {
  ...
  return(list(h=h, date_time=date_time))
}
...
eva_data <- eva_rate_cal(h, date_time)
...
df <- data.frame(eva_date_time, eva_rate=eva_rate)
df_precip <- data.frame(t=t, P_d=-1 * P_d * 25.4 / 24)
df_precip$t <- as.POSIXct(df_precip$t, format = "%Y-%m-%d", tz = "UTC")
...
ggplot() +
  ...
  geom_bar(data=df_precip, aes(x=t, y=P_d), stat="identity", fill="red", color="red") +
