...
This code implements the Webb model for calculating Mono Lake evaporation rates. This approach combines various environmental data such as air temperature, wind speed, solar radiation, and salinity, alongside the evaporation rates measured on-site. The conversion calculations are mostly based on meteorological and water properties. The script also integrates the raw pan evaporation rates for validation. The main goal is to establish a understanding of the evaporation rates from a vapor pressure gradient standpoint, with salinity effects on lake thermal properties and temperature variations being considered. The study will also explore the evaporation coefficient as it changes over time and how it's sensitive to lake water temperature,  
...
You can also define the range of dates you want to compute.
...
Here are some helper functions to simplify the main code. You don't need to change any of the settings.
...
# Load Data from Climate Stations
...
Different from other daily data from climate stations, we failed to obtain such data so monthly average from past research is used and loaded here.
...
# Webb Method Conversion Calculation

The core of this code script is calculating the vapor pressure gradient between air and water for both Mono Lake and evaporation pan. The model utilizes various climate variables such as air and water temperatures, solar radiation, and wind speed, as well as water properties like salinity for thermal properties adjustment.
...
The conversion calculated assuming the gradients are proportional. Both converted evaporation rate and evaporation coefficients are generated. 
...
write.csv(output_coeff, file.path(global_work_dir, "output", "eva_coeff_Webb_RStudio.csv"), row.names = FALSE)
```
